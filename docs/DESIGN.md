...
- **Cell–cell collision**: true capsule–capsule min distance; soft spring +
  ...
  an immovable collision proxy** (the assembly parent, inverse mass 0) until the
  daughters separate.
...
- **Mitosis fade** scales the parent's instance matrix down to zero (`fade` in
  ...
  **immovable collision proxy** (the assembly parent, recentered on `asm.startPos`) until

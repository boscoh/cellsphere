...
- `src/components/PerfPanel.vue` — perf breakdown panel; `src/components/PopChart.vue` — population chart (time / phase); `src/components/RateChart.vue` — measured per-capita rate panel (time / rate plane) (`rateModel.js`, `popChartMath.js`, `chartCanvas.js` are its pure data/canvas helpers)

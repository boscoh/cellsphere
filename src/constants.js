...
export const PARAM_DEFS = [
  ...
  { key: 'MITO_SEP', group: 'mitosis', label: 'Mito sep', desc: 'Final separation (x half child length) at division release. The daughters ease from MITO_NEAR to MITO_SEP over MITO_DRIFT.', def: 2.45, min: 0.5, max: 8, step: 0.05 },
  { key: 'MITO_DRIFT', group: 'mitosis', label: 'Mito drift', desc: 'Fraction of MITO_TIME the daughters spend easing from MITO_NEAR to MITO_SEP, starting when the handover ends (MITO_HOLD + MITO_FADE). The division releases at the end of the drift, so there is no frozen daughter time; 0 releases right at the end of the handover.', def: 0.2, min: 0, max: 1, step: 0.05 },
  ...
  { key: 'MITO_AURA', group: 'mitosis', label: 'Mito aura', desc: 'Cosmetic gate on the assembly aura channel: 1 ramps the rim marker in while a dividing unit is being eaten and out when the bites stop, 0 forces it off. The predator-latched rim glow is unaffected.', def: 1, min: 0, max: 1, step: 1 },

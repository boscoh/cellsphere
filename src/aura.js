...
// Glow half-length along the body axis and half-width across it, as a uniform
// scale of the capsule (radius x width) so the halo keeps the body's oval
// aspect instead of rounding out to a circle.
const LEN_K = 1.5
const WIDTH_K = 1.5
...
export function renderAura(sim) {
  ...
  for (const d of sim.cells) {
    ...
    halfWidth.setX(n, d.width * WIDTH_K)
